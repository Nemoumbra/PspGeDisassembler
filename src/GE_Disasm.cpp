...
void GE_Disasm::GeDisassembleOp(u32 pc, u32 op, u32 prev, char *buffer, int bufsize) {
    ...
    switch (cmd)
    {
        ...
        case GE_CMD_VADDR:
            snprintf(buffer, bufsize, "VADDR: %06x => %08x", data, getRelativeAddress(data));
            break;

        case GE_CMD_IADDR:
            snprintf(buffer, bufsize, "IADDR: %06x => %08x", data, getRelativeAddress(data));
            break;
        ...
        case GE_CMD_JUMP:
        {
            u32 target = getRelativeAddress(op & 0x00FFFFFC);
                ...
                case GE_CMD_SIGNAL:
                {
                    ...
                    switch (behaviour) {
                        ...
                        case PSP_GE_SIGNAL_OJUMP:
                            target = getRelativeAddress(target);
